...
UpdateStatsAttDefNHL<- function(){
  ...
  theurl <- getURL("https://www.hockey-reference.com/leagues/NHL_2020_skaters.html",.opts = list(ssl.verifypeer = FALSE) )
...
UpdateStatsGardiens<- function(){
  ...
  theurl <- getURL("https://www.hockey-reference.com/leagues/NHL_2020_goalies.html",.opts = list(ssl.verifypeer = FALSE) )
  tables <- readHTMLTable(theurl)
  tables <- list.clean(tables, fun = is.null, recursive = FALSE)
  n.rows <- unlist(lapply(tables, function(t) dim(t)[1]))
