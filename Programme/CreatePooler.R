...
createAleatoireTeam<-function(){
  ...
  nomAtt<-sample_n(subset(statsJoueurs, (Position == "LW" | Position == "RW" | Position == "C") & GP >= 50 , c(Joueurs, Equipe)),15)
  nomDef<-sample_n(subset(statsJoueurs, Position == "D" &  GP >= 50 , c(Joueurs, Equipe)),8)
  statsGardiens<- dbReadTable(con, "statsGardiens")
  nomGardien<-sample_n(subset(statsGardiens, GP >= 40 , c(Joueurs, Equipe)),3)
  POS<-c(rep("Att",15), rep("Def",8), rep("G",3))
  Statue<- c(rep("New", 26))
...
createPoolers <- function(poolerName, poolerColor, infoAlignement, password){
  ...
  infoAttInit <- data.frame(cbind(infoAtt, matrix(rep(0,90), ncol = 6),matrix(rep(as.character(Sys.Date( )),30), ncol = 2)))
  ...
  infoDefInit <- data.frame(cbind(infoDef, matrix(rep(0,48), ncol = 6),matrix(rep(as.character(Sys.Date( )),16), ncol = 2)))
